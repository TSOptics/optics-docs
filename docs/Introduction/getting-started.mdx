...
- Install the React version (requires React 18+):

```bash npm2yarn
npm install @optics/react
```

- **Or** install the framework agnostic core:

```bash
npm install @optics/state
```

This library has been built around TypeScript's powerful inference capabilities, using it in strict mode is strongly recommended.
...
An optic is a **reference** to a piece of application state and it allows us to read, update this piece as well as subscribe to it.
...
// @include: main
// ---cut---
onUsers[0].address.city.get(); // "Boston"
...
const onJaneStreet = onUsers[1].address.street;
// ---cut---
<input onChange={(e) => onJaneStreet.name.set(e.target.value)} />;
```
...
```ts twoslash
// @include: main
const onJane = onUsers[1];
// ---cut---

onJane.age.set((prev) => prev + 1);
// jane's street reference hasn't changed, StreetForm doesn't re-render
```
