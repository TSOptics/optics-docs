...
const onJeanneStreetTuple = onJeanne.address.street.derive(
...
const onJeanne = onUsers[1];
// ---cut---
const onStreet = onJeanne.address.street;

const onTuple = onStreet.derive(({ name, number }) => [name, number] as const);
//    ^?
```
