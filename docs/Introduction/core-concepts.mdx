...
A combinator is simply a function that returns an object with a get and a set, that you can call inside `derive` to get a new optic.  
The library exports several of these functions under `@optics/react/combinators`, like `find`, `filter`, `cond` or `max`.
