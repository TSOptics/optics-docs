...
const oldestUserOptic = usersOptic.max((user) => user.age);
...
The [`max`](<../API/Methods/array/max()>), [`min`](<../API/Methods/array/min()>) or [`findFirst`](<../API/Methods/array/findFirst()>) methods are available when the focused type is an array, the [`entries`](<../API/Methods/object/entries()>) and [`values`](<../API/Methods/object/values()>) methods when it's a record,
and some methods are available for all optics like [`if`](<../API/Methods/if()>) or [`refine`](<../API/Methods/refine()>).

They all could be implemented with the [`derive`](<../API/Methods/derive()>) method, which lets you manually derive new optics.
It takes two parameters, a `get` function to derive a new value from the original one (sometimes called a **selector**), and a `set` to update the original value from the derived one:
...
// @include: Interpol
// ---cut---
const interpol = interpolOptic.get();
// {
//   name: "Interpol",
//   established: "1923",
//   employees: [
...
// @include: Interpol
// ---cut---
const result = interpolOptic.get({ denormalize: false });
```
...
```ts twoslash
// @include: Interpol
const denormalizedResult = interpolOptic.get();
