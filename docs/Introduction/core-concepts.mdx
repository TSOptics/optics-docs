...
They point to parts of your application's immutable state and let's you interact with it.  
...
When we access the index `1` of the root optic and then the `age` property, getting us a new optic focused on a narrower part of the initial state.  
...
:::caution
Don't use destructuring to derive new optics, since we use Proxies under the hood to return new optics from properties the following won't work:
...
One such method is [`convert`](<../API/Methods/convert()>). It let's you perform a lossless conversion from the focused type to another.  
For example if we wanted to manipulate the user's street as a tuple instead of an object:

```ts
const onJeanStreetTuple = onJeanne.address.street.convert(
...
```ts
onJeanStreetTuple.set(([name, number]) => [name, number + 10]);
...
To illustrate let's create a new state, a club with a name and a list of members:

```ts
const onClub42 = createState({
  name: "Club 42",
  established: "1974",
...
As you can see the members we passed are not actual javascript objects but optics we created earlier.

When we call `get` on the club's optic it automatically **denormalizes** the result and replaces the optics by their respective state:
...
If we update the state of one of the members then it's like if the club state was updated as well
so its subscribers will be notified (will re-render if they are components).

To illustrate let's give back Jeanne her youth:

```ts
onClub42.members.subscribe((members) =>
...
```
one of the 2 members was updated
```
...
```ts
onClub42.get({ denormalize: false });
```

The returned type will of course change depending on whether denormalize is set to
true or false:
...
The club has a reference to its members but in turn indivual members can also have references to other entities.  
...
Now the fully denormalized result from `onClub42` looks like that:
...
where the **nodes** are the entities we build with `createState` and the **edges** are the optics relating them together.

While immutable state is great it however makes it hard to represent such graphs.  
We usually have to resort to representing the relation with the the id of the target entity and then essentially joining the entities manually to get our denormalized result.  
Optics makes the whole process **automatic**, **reactive** and **typesafe**, which is fundamental for applications that actually need client side state management,
as most of them will eventually need to represent relations between entities.
...
And that's where lies the problem, **the component shouldn't have know the shape of the global state**.  
It's only job should be rendering a user, no matter where it comes from.

:::info
While we could split our component into _smart_ and _dumb_ ones (or _container_ and _presentational_) to keep the dumb component decoupled from the state,
this pattern has fallen out of favour since the introduction of hooks due its verbosity, and the additional nesting it piles up in the component tree.
:::
...
Listing dependencies in the props is always recommanded to avoid coupling and there's no reason that it should be otherwise for global state !
...
Thus components close to the root usually import optics directly:
...
But now that we have the optic imported we can derive new ones from it and pass them down to the `User` elements.  
In turn we can imagine that the `User` component derives an optic focused on the user's street and pass it down to a `StreetForm` element, etc ...

That way **most leaf components can be decoupled** from the global state, which is convenient as they are the ones that usually need to be reused, tested or documented in stories.
...
To further your knowledge you can look through the following guides:

- [Total vs partial](../Guides/total_partial): understand how a total optic will always yield a value while it might not be the case for a partial one.
- [map/reduce](../Guides/map_reduce): learn how to focus multiple values at a time, and then how to **reduce** the focus back to a single value.
