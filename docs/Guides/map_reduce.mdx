...
The `For` component iterates over the values of a mapped optic and gives you a new optic for each of them:
...
const Attractions = () => {
  return (
    <For mappedOptic={attractionsOptic} getKey={(name) => name}>
      {(attractionOptic) => <Attraction attractionOptic={attractionOptic} />}
    </For>
  );
};
```
