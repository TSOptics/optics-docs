...
Calling `.get()` returns a flat representation of all attractions, even though in the state they are nested in their respective national parks.
...
They yield new optics, either mapped or partial, that **reduce** the mapped values to focus on some of them or back to a single one.
...
```ts
const onFirstThree = onSortedAttractions.reduceSlice(0, 3);
...
```ts
onNationalParks.attractions.max((attraction) => attraction.length);
// Optic<string, mapped>
...
```ts
onNationalParks.attractions.reduceMax((attractions) => attractions.length);
// Optic<string[], partial>

// ['Half Dome', 'El Capitan', 'Mariposa Grove']
```

With the first example we focus on the attraction with the longest name for each national park,
while with the second example we focus on the attractions of the national park that has the most attractions (Yosemite).
